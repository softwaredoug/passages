...
def test_set_get_float64_default(redis_mock):
    ...
    vector_cache = VectorCache(redis_mock, dims=2)
...
def test_none_on_wrong_dims(redis_mock):
    vector_cache = VectorCache(redis_mock, dtype=np.float64, dims=3)
    arr = np.array([1.0, 2.0, 3.0])
    vector_cache.set("foo", arr)

    read_vector_cache = VectorCache(redis_mock, dtype=np.float64, dims=1)
    foo = read_vector_cache.get("foo")
    assert foo is None
